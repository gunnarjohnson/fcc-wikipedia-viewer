...
function clearResults() {
	document.getElementById("wikipedia-viewer--results").innerHTML = "";
}

// Searches Wikipedia for pages matching form input
function searchWikipedia() {
  // Assigns form input to variable
  var searchInput = document.getElementById("wikipedia-search--input").value;
	// Executes if form input is empty
	if (searchInput == "") {
  	var div = document.createElement("div");
    document.getElementById("wikipedia-viewer--results").appendChild(div);
    // Assigns div class name
    div.className = "wikipedia-search--results";
		...
		$.ajax({
			...
			success: function(data) {
				...
				for (i = 0; i < 10; i++) {
					...
					div.id = "wikipedia-search--results---" + i;
...
$("#wikipedia-search--input").keyup(function hitEnter(event){
    if(event.keyCode == 13){
        $("#wikipedia-search--button").click();
    }
});

// Calls functions when button is clicked
var searchButton = document.getElementById("wikipedia-search--button");
